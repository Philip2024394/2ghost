...
export default function AffiliateJoinPage() {
  ...
  const handleSubmit = () => {
    ...
    upsertAffiliate({
      ...
      whatsapp:       wa.trim(),
      city:           city.trim(),
      status:         "active",   // live immediately — no approval needed
      createdAt:      Date.now(),
      commissionRate: 25,
    });
    setCode(newCode);
        ...
        {!done ? (
          <>
            ...
            <div style={{ background: "rgba(74,222,128,0.05)", border: "1px solid rgba(74,222,128,0.12)", borderRadius: 12, padding: "12px 14px", marginBottom: 18 }}>
              {[
                ["🔗", "Your own share link — active the moment you sign up"],
                ["🖼️", "Ready-made banners + promo videos to post"],
                ["📊", "Live dashboard — clicks, signups, income"],
                ["💸", "25% commission — paid monthly via GoPay/OVO/Transfer"],
              ].map(([icon, text]) => (
            ...
            {/* Earnings example */}
            <div style={{ background: "rgba(250,204,21,0.05)", border: "1px solid rgba(250,204,21,0.15)", borderRadius: 10, padding: "10px 14px", marginBottom: 18 }}>
              ...
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", margin: 0, lineHeight: 1.6 }}>
                5 paid conversions = <strong style={{ color: "#fbbf24" }}>202,500 IDR</strong>/month<br />
                20 paid conversions = <strong style={{ color: "#fbbf24" }}>810,000 IDR</strong>/month
              </p>
            </div>
            ...
            >
              Join Now — Get My Link →
            </button>
          </>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: "center" }}>
            <div style={{ fontSize: 48, marginBottom: 10 }}>🎉</div>
            <h2 style={{ fontSize: 20, fontWeight: 900, color: "#4ade80", margin: "0 0 6px" }}>You're Live!</h2>
            <p style={{ fontSize: 12, color: "rgba(255,255,255,0.45)", margin: "0 0 20px", lineHeight: 1.6 }}>
              Your affiliate account is active. Start sharing your link now.
            </p>

            <div style={{ background: "rgba(74,222,128,0.08)", border: "1px solid rgba(74,222,128,0.25)", borderRadius: 14, padding: "16px 14px", marginBottom: 16, textAlign: "left" }}>
              <p style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", margin: "0 0 6px", fontWeight: 700, letterSpacing: 1 }}>YOUR CODE</p>
              <p style={{ fontSize: 26, fontWeight: 900, color: "#4ade80", margin: "0 0 10px", letterSpacing: 4 }}>{code}</p>
              <p style={{ fontSize: 10, color: "rgba(255,255,255,0.35)", margin: "0 0 10px", wordBreak: "break-all", lineHeight: 1.5 }}>{shareLink}</p>
              <button
                onClick={copy}
                style={{
                  width: "100%",

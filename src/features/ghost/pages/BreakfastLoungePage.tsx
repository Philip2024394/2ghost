...
export default function BreakfastLoungePage() {
  ...
  // ── Chat likes (kept for browsing view coffee reply categories)
  // Group table state moved to BreakfastTableChat
  ...
  // Chat state moved to BreakfastTableChat — only rate limiter ref kept for sendCoffee
  ...
  const handleSendInvite = useCallback(() => {
    ...
    setTimeout(() => {
      if (accept) {
        setPhase("at-table"); // BreakfastTableChat initialises its own chat
      } else {
  ...
  // inviteThird moved to BreakfastTableChat
  ...
  // sendChat, tipButler, leaveTable, confirmLeave moved to BreakfastTableChat
  ...
  if (phase === "at-table" && partner) {
    return (
      <BreakfastTableChat
        ...
        available={visible}
        onLeave={() => { setPhase("browsing"); setPartner(null); }}
      />
    );
  }

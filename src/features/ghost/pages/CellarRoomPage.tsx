...
// ── Cellar theme ──────────────────────────────────────────────────────────────
const C          = "#c0392b";
const C_GLOW     = (o: number) => `rgba(192,57,43,${o})`;
const C_GRAD     = "linear-gradient(to bottom, #e74c3c 0%, #c0392b 40%, #7b241c 100%)";
...
// ── Profiles ──────────────────────────────────────────────────────────────────
const CELLAR_PROFILES: GhostProfile[] = INTL_PROFILES.map((p, i) => ({
  ...p,
  id: `cellar-${p.id}`,
  last_seen_at: i % 3 !== 0
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function CellarRoomPage() {
  const navigate = useNavigate();
  const [likedIds,           setLikedIds]        = useState<Set<string>>(new Set());
  const [memberTab,          setMemberTab]        = useState<MemberTab>("members");
  const [showChat,           setShowChat]         = useState(false);
  const [chatUnread,         setChatUnreadSt]     = useState(() => getChatUnread("cellar"));
  ...
  const [showButlerArrival,  setShowButlerArrival] = useState(() => isFirstEntry("cellar"));
  ...
  const likedMeProfiles = useMemo(
    () => CELLAR_PROFILES.filter((_, i) => i % 2 === 0).slice(0, 3),
    [],
  );
  const iLikeProfiles = useMemo(
    () => CELLAR_PROFILES.filter(p => likedIds.has(p.id)),
    [likedIds],
  );

  const tabLabel = memberTab === "ilike" ? "I Like" : memberTab === "liked" ? "Liked Me" : "The Cellar";
  const tabCount = memberTab === "ilike" ? iLikeProfiles.length
    : memberTab === "liked" ? likedMeProfiles.length
    : CELLAR_PROFILES.length;

  // Filtered profiles based on country selection
  const displayProfiles = useMemo(() => {
    if (!browsingCountryCode) return CELLAR_PROFILES;
    return CELLAR_PROFILES.filter(p =>
      (p as GhostProfile & { countryCode?: string }).countryCode === browsingCountryCode,
    ).concat(CELLAR_PROFILES.slice(0, 4)); // always show some
  }, [browsingCountryCode]);
  ...
  const renderScrollContent = () => {
    ...
    // Default: Cellar Members
    return (
      <>
        {CELLAR_PROFILES.slice(0, 12).map((p, i) => {
    ...
    <div style={{ minHeight: "100dvh", background: "#06060a", color: "#fff", fontFamily: "inherit", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ margin: "10px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
          <style>{`.cellar-hero-row::-webkit-scrollbar { display: none; }`}</style>
          <div className="cellar-hero-row" style={{ flex: 1, display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, scrollbarWidth: "none" }}>
            {renderScrollContent()}
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <FloorChatPopup tier="cellar" tierColor={C} tierLabel="The Cellar" tierIcon="🍷" onClose={() => setShowChat(false)} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showButlerArrival && (
          <ButlerArrivalPopup
            floor="cellar" floorLabel="The Cellar" floorColor={C} floorIcon="🍷"
      ...
      <AnimatePresence>
        {showGuestPicker && (
          <BreakfastGuestPicker
            floor="cellar" profiles={CELLAR_PROFILES}
      ...
      <AnimatePresence>
        {showLoungeSplash && (
          <LoungeSplashScreen
            floorLabel="The Cellar" floorColor={C} floorIcon="🍷"
      ...
      <AnimatePresence>
        {showViewedMe && (
          <FloorViewedMePopup
            floorKey="cellar" floorLabel="The Cellar" floorColor={C}
            profiles={CELLAR_PROFILES}

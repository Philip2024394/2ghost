...
export default function GhostModePage() {
  ...
  // For unsubscribed users: holds the matched profile until they pay
  const [matchPaywallProfile, setMatchPaywallProfile] = useState<GhostProfile | null>(null);
  // After payment: show connect-now screen with app options
  ...
  return (
    ...
    <div style={{ width: "100%", maxWidth: 480, minHeight: "100dvh", background: "#050508", color: "#fff", display: "flex", flexDirection: "column", position: "relative" }}>
      ...
      <AnimatePresence>
        {matchProfile && (
          <GhostMatchPopup
            ...
            onConnectWhatsApp={() => {
              if (isGhost) {
                // Subscriber — show connect screen directly (already paid)
                if (matchProfile) {
          ...
          <MatchPaywallModal
            ...
            onPay={(planKey) => {
              ...
              if (profile) {
                ...
                setConnectNowProfile(profile); // ← show connect screen
              }
            }}
      ...
      {/* ── Connect Now screen — shown after payment or subscriber connect ── */}
      <AnimatePresence>
        {connectNowProfile && (
          <ConnectNowPopup

...
// ── Loft theme ────────────────────────────────────────────────────────────────
const C          = "#8b5cf6";
const C_GLOW     = (o: number) => `rgba(139,92,246,${o})`;
const C_GRAD     = "linear-gradient(to bottom, #a78bfa 0%, #8b5cf6 40%, #5b21b6 100%)";
...
// ── Profiles ──────────────────────────────────────────────────────────────────
const LOFT_PROFILES: GhostProfile[] = INTL_PROFILES.map((p, i) => ({
  ...p,
  id: `loft-${p.id}`,
  last_seen_at: i % 3 !== 0
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function LoftRoomPage() {
  const navigate = useNavigate();
  const [likedIds,           setLikedIds]        = useState<Set<string>>(new Set());
  const [memberTab,          setMemberTab]        = useState<MemberTab>("members");
  const [showChat,           setShowChat]         = useState(false);
  const [chatUnread,         setChatUnreadSt]     = useState(() => getChatUnread("loft"));
  ...
  const [showButlerArrival,  setShowButlerArrival] = useState(() => isFirstEntry("loft"));
  ...
  const likedMeProfiles = useMemo(
    () => LOFT_PROFILES.filter((_, i) => i % 2 === 0).slice(0, 3),
    [],
  );
  const iLikeProfiles = useMemo(
    () => LOFT_PROFILES.filter(p => likedIds.has(p.id)),
    [likedIds],
  );

  const tabLabel = memberTab === "ilike" ? "I Like" : memberTab === "liked" ? "Liked Me" : "The Loft";
  const tabCount = memberTab === "ilike" ? iLikeProfiles.length
    : memberTab === "liked" ? likedMeProfiles.length
    : LOFT_PROFILES.length;

  // Filtered profiles based on country selection
  const displayProfiles = useMemo(() => {
    if (!browsingCountryCode) return LOFT_PROFILES;
    return LOFT_PROFILES.filter(p =>
      (p as GhostProfile & { countryCode?: string }).countryCode === browsingCountryCode,
    ).concat(LOFT_PROFILES.slice(0, 4)); // always show some
  }, [browsingCountryCode]);
  ...
  const renderScrollContent = () => {
    ...
    // Default: Loft Members
    return (
      <>
        {LOFT_PROFILES.slice(0, 12).map((p, i) => {
    ...
    <div style={{ minHeight: "100dvh", background: "#06060a", color: "#fff", fontFamily: "inherit", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ margin: "10px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
          <style>{`.loft-hero-row::-webkit-scrollbar { display: none; }`}</style>
          <div className="loft-hero-row" style={{ flex: 1, display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, scrollbarWidth: "none" }}>
            {renderScrollContent()}
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <FloorChatPopup tier="loft" tierColor={C} tierLabel="The Loft" tierIcon="🎨" onClose={() => setShowChat(false)} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showButlerArrival && (
          <ButlerArrivalPopup
            floor="loft" floorLabel="The Loft" floorColor={C} floorIcon="🎨"
      ...
      <AnimatePresence>
        {showGuestPicker && (
          <BreakfastGuestPicker
            floor="loft" profiles={LOFT_PROFILES}
      ...
      <AnimatePresence>
        {showLoungeSplash && (
          <LoungeSplashScreen
            floorLabel="The Loft" floorColor={C} floorIcon="🎨"
      ...
      <AnimatePresence>
        {showViewedMe && (
          <FloorViewedMePopup
            floorKey="loft" floorLabel="The Loft" floorColor={C}

...
// ── Penthouse theme ───────────────────────────────────────────────────────────
const C          = "#c0c8d8";
const C_GLOW     = (o: number) => `rgba(192,200,216,${o})`;
const C_GRAD     = "linear-gradient(to bottom, #e8edf5 0%, #c0c8d8 40%, #7a8494 100%)";
...
// ── Profiles ──────────────────────────────────────────────────────────────────
const PENT_PROFILES: GhostProfile[] = INTL_PROFILES.map((p, i) => ({
  ...p,
  id: `pent-${p.id}`,
  last_seen_at: i % 3 !== 0
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function PenthouseRoomPage() {
  const navigate = useNavigate();
  const [likedIds,           setLikedIds]        = useState<Set<string>>(new Set());
  const [memberTab,          setMemberTab]        = useState<MemberTab>("members");
  const [showChat,           setShowChat]         = useState(false);
  const [chatUnread,         setChatUnreadSt]     = useState(() => getChatUnread("penthouse"));
  ...
  const [showButlerArrival,  setShowButlerArrival] = useState(() => isFirstEntry("penthouse"));
  ...
  const likedMeProfiles = useMemo(
    () => PENT_PROFILES.filter((_, i) => i % 2 === 0).slice(0, 3),
    [],
  );
  const iLikeProfiles = useMemo(
    () => PENT_PROFILES.filter(p => likedIds.has(p.id)),
    [likedIds],
  );

  const tabLabel = memberTab === "ilike" ? "I Like" : memberTab === "liked" ? "Liked Me" : "The Penthouse";
  const tabCount = memberTab === "ilike" ? iLikeProfiles.length
    : memberTab === "liked" ? likedMeProfiles.length
    : PENT_PROFILES.length;

  // Filtered profiles based on country selection
  const displayProfiles = useMemo(() => {
    if (!browsingCountryCode) return PENT_PROFILES;
    return PENT_PROFILES.filter(p =>
      (p as GhostProfile & { countryCode?: string }).countryCode === browsingCountryCode,
    ).concat(PENT_PROFILES.slice(0, 4)); // always show some
  }, [browsingCountryCode]);
  ...
  const renderScrollContent = () => {
    ...
    // Default: Penthouse Members
    return (
      <>
        {PENT_PROFILES.slice(0, 12).map((p, i) => {
    ...
    <div style={{ minHeight: "100dvh", background: "#06060a", color: "#fff", fontFamily: "inherit", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ margin: "10px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
          <style>{`.penthouse-hero-row::-webkit-scrollbar { display: none; }`}</style>
          <div className="penthouse-hero-row" style={{ flex: 1, display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, scrollbarWidth: "none" }}>
            {renderScrollContent()}
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <FloorChatPopup tier="penthouse" tierColor={C} tierLabel="The Penthouse" tierIcon="🏙️" onClose={() => setShowChat(false)} />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showButlerArrival && (
          <ButlerArrivalPopup
            floor="penthouse" floorLabel="The Penthouse" floorColor={C} floorIcon="🏙️"
      ...
      <AnimatePresence>
        {showGuestPicker && (
          <BreakfastGuestPicker
            floor="penthouse" profiles={PENT_PROFILES}
      ...
      <AnimatePresence>
        {showLoungeSplash && (
          <LoungeSplashScreen
            floorLabel="The Penthouse" floorColor={C} floorIcon="🏙️"
      ...
      <AnimatePresence>
        {showViewedMe && (
          <FloorViewedMePopup
            floorKey="penthouse" floorLabel="The Penthouse" floorColor={C}
            profiles={PENT_PROFILES}

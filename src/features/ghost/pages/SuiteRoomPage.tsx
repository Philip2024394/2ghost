...
// ── Suite theme ───────────────────────────────────────────────────────────────
const C          = "#cd7f32";
const C_GLOW     = (o: number) => `rgba(205,127,50,${o})`;
const C_GRAD     = "linear-gradient(to bottom, #e8a050 0%, #cd7f32 40%, #7a3b10 100%)";
const SUITE_BG   = "https://ik.imagekit.io/7grri5v7d/asdfasdfasdwqdssdsdewtrewrtdsds.png";
const GHOST_LOGO = "https://ik.imagekit.io/7grri5v7d/Untitledsdfasdfasdf-removebg-preview.png?updatedAt=1774209542052";
...
// ── Profiles ──────────────────────────────────────────────────────────────────
const SUITE_PROFILES: GhostProfile[] = INTL_PROFILES.map((p, i) => ({
  ...p,
  id: `suite-${p.id}`,
  last_seen_at: i % 3 !== 0
...
export default function SuiteRoomPage() {
  ...
  const [chatUnread,         setChatUnreadSt]     = useState(() => getChatUnread("suite"));
  ...
  const [showButlerArrival,  setShowButlerArrival] = useState(() => isFirstEntry("suite"));
  ...
  const likedMeProfiles = useMemo(
    () => SUITE_PROFILES.filter((_, i) => i % 2 === 0).slice(0, 3),
    [],
  );
  const iLikeProfiles = useMemo(
    () => SUITE_PROFILES.filter(p => likedIds.has(p.id)),
    [likedIds],
  );
  ...
  const displayProfiles = useMemo(() => {
    if (!browsingCountryCode) return SUITE_PROFILES;
    return SUITE_PROFILES.filter(p =>
      (p as GhostProfile & { countryCode?: string }).countryCode === browsingCountryCode,
    ).concat(SUITE_PROFILES.slice(0, 4)); // always show some
  }, [browsingCountryCode]);
  ...
  const renderScrollContent = () => {
    ...
    // Default: Suite Members
    return (
      <>
        {SUITE_PROFILES.slice(0, 12).map((p, i) => {
    ...
    <div style={{ minHeight: "100dvh", background: "#06060a", color: "#fff", fontFamily: "inherit", position: "relative", overflowX: "hidden" }}>
      ...
      <div style={{ margin: "10px 14px 0" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
          <style>{`.suite-hero-row::-webkit-scrollbar { display: none; }`}</style>
          <div className="suite-hero-row" style={{ flex: 1, display: "flex", gap: 8, overflowX: "auto", paddingBottom: 8, scrollbarWidth: "none" }}>
            {renderScrollContent()}
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          <FloorChatPopup
            ...
            breakfastHostName={breakfastInviteRef?.fromUserName}
            onEndBreakfast={isBreakfastChat ? () => {
              setShowChat(false);
              setShowRating(true);
            } : undefined}
            onClose={() => { setShowChat(false); setIsBreakfastChat(false); }}
          />
        )}
      </AnimatePresence>
      ...
      {/* ── Butler Arrival Popup (first entry only) ── */}
      <AnimatePresence>
        {showButlerArrival && (
          <ButlerArrivalPopup
      ...
      <AnimatePresence>
        {receivedInvite && (
          <BreakfastInviteReceived
            ...
            onAccept={() => {
              setBreakfastInviteRef(receivedInvite);
      ...
      {/* ── Lounge Splash (5 sec transition) ── */}
      <AnimatePresence>
        {showLoungeSplash && (
          <LoungeSplashScreen
            floorLabel="The Suite" floorColor={C} floorIcon="🛎️"
            guestName={loungeGuestName}
            onDone={() => { setShowLoungeSplash(false); setIsBreakfastChat(true); setShowChat(true); }}
          />
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showRating && breakfastInviteRef && (
          <BreakfastRatingPopup
            floorColor={C} floorLabel="The Suite" guestName={loungeGuestName}
            onRated={(rating) => {
              setShowRating(false);
      ...
      {/* ── Butler Match Signal + Activity Picker ── */}
      <AnimatePresence>
        {showMatchSignal && breakfastInviteRef && (
          <ButlerMatchSignalPopup
            ...
            floor="suite" floorColor={C} floorLabel="The Suite"
            onClose={() => setShowMatchSignal(false)}
          />
        )}
      </AnimatePresence>

      {/* ── Social Activity Invite (24h delivery) ── */}
      <AnimatePresence>
        {socialInvite && (
      ...
      {/* ── Butler excuse (proposed breakfast time missed) ── */}
      <AnimatePresence>
        {showExcuse && (
          <ButlerExcusePopup
            invite={showExcuse}
      ...
      <AnimatePresence>
        {showViewedMe && (
          <FloorViewedMePopup
            floorKey="suite" floorLabel="The Suite" floorColor={C}
            profiles={SUITE_PROFILES}
